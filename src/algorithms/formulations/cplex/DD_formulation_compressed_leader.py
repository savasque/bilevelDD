...
def get_model(instance, diagram, time_limit, incumbent):
    ...
    w = model.continuous_var_dict([arc.id for arc in arcs], ub=1, name="w")
    ...
    beta = model.binary_var_dict([(arc.id, idx) for arc in arcs for idx in interaction_indices if arc.player == "leader"], name="beta")
    ...
    model.add_constraint_(model.sum(d[j] * y[j] for j in range(Fcols)) <= model.sum(arc.cost * w[arc.id] for arc in arcs), ctname="value-function")

    # Flow constrs
    model.add_constraint_(model.sum(w[arc.id] for arc in root_node.outgoing_arcs) - model.sum(w[arc.id] for arc in root_node.incoming_arcs) == 1, ctname="flow-cons-root")
    model.add_constraint_(model.sum(w[arc.id] for arc in sink_node.outgoing_arcs) - model.sum(w[arc.id] for arc in sink_node.incoming_arcs) == -1, ctname="flow-cons-sink")
    model.add_constraints_((model.sum(w[arc.id] for arc in node.outgoing_arcs) - model.sum(w[arc.id] for arc in node.incoming_arcs) == 0 for node in nodes if node.id not in ["root", "sink"]), names="flow-cons")
